(() => {
  ...
  function loadGlobeScripts() {
    const scripts = [
      ...
      "https://static.radar.com/scripts/globe_data.min.js",
    ];
  ...
  const target = document.querySelector("#globe-section");
  if (target) {
    const io = new IntersectionObserver((entries, obs) => {
      entries.forEach(entry => {
        if (entry.isIntersecting && entry.intersectionRatio >= 0.05) {
          loadGlobeScripts();
          obs.unobserve(target); // load once
        }
      });
    }, { threshold: [0.05] });
...
const initGlobe = () => {
  ...
  const filterCitiesPercentage = 0.5;
  const overlap = 60;
  ...
  for (let i = 0; i < filteredCities.length; i++) {
    const start = Math.floor(Math.random() * overlap);
  ...
  const setControls = (globe) => {
    ...
    globe.pointOfView({ lat: 40, lng: -95, altitude: startingZoom }); // Center on North America
